...
type SignupFormProps = { disabled?: boolean };

function FormInput({
  id, name, type, autoComplete, placeholder, value, onChange, onBlur, error, icon: Icon,
}: {
  id: string; name: string; type: string; autoComplete: string; placeholder: string;
...
export function SignupForm({ disabled = false }: SignupFormProps) {
  ...
  const validate = useCallback((field: string, newValues: typeof values) => {
    const result = signUpSchema.safeParse(newValues);
    if (result.success) {
